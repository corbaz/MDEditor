...
function getJsonStorePath() {
  if (!jsonStorePath) {
    jsonStorePath = path.join(app.getPath('userData'), 'md-editor-state.json')
  }
  ...
  try {
    ...
    for (const table of ['documents', 'recent_documents']) {
      for (const column of [
        ...
        try {
          sqliteDatabase.exec(`ALTER TABLE ${table} ADD COLUMN ${column[0]} ${column[1]};`)
        } catch {
...
async function saveLatestDocument(document) {
  ...
  const previousFilename = document.previousFilename ? String(document.previousFilename) : ''
  ...
  if (database) {
    database
      ...
      `)
      .run(filename, markdown, updatedAt, metadata.filePath, metadata.folderPath, metadata.sizeBytes)
  ...
  return { filename, markdown, previousFilename: previousFilename || previousJson?.filename, ...metadata }
}
...
async function loadRecentDocuments() {
  ...
  if (database) {
    ...
    for (const row of rows) {
      if (await pathExists(row.filePath)) {
        ...
        clearDocumentFromStore({ filename: row.filename, filePath: row.filePath })
      }
  ...
  return latest?.filePath && await pathExists(latest.filePath)
    ? [{ filename: latest.filename, updatedAt: latest.updatedAt, filePath: latest.filePath, folderPath: latest.folderPath, sizeBytes: latest.sizeBytes }]
    : []
}
...
async function loadRecentDocument(filename) {
  ...
  const latest = await loadLatestDocument()
  return latest?.filename === normalizedFilename && await pathExists(latest.filePath) ? latest : null
}
...
ipcMain.handle('save-image-file', async (_event, image) => {
  ...
  const ext = EXT_BY_MIME[mime] || path.extname(originalName).toLowerCase() || '.png'
  const safeName = path
